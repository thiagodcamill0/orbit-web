...
function bindCanvas(): void {
  ...
  viewport.addEventListener('mousedown', (e: MouseEvent) => {
    if (e.button !== 0 || node.dragging) return;

    const onNode = node.el!.contains(e.target as Node);
...
function bindNode(): void {
  ...
  el.addEventListener('mousedown', (e: MouseEvent) => {
    ...
    if ((e.target as Element).closest('.agent-action')) return;
  ...
  window.addEventListener('mouseup', () => {
    ...
    if ((el as any)._quickClick && !editMode) {
      enterEditMode(); // quick click = select agent, enter edit mode
    }
    (el as any)._quickClick = false;
...
function handleAction(action: string): void {
  switch (action) {
    case 'name':
      openPanel();
      setTimeout(() => (document.getElementById('fieldName') as HTMLInputElement)?.focus(), 320);
      break;
    case 'prompt':
      openPanel();
      setTimeout(() => (document.getElementById('fieldPrompt') as HTMLTextAreaElement)?.focus(), 320);
      break;
    case 'status': {
...
function bindButtons(): void {
  function zoomCenter(step: number): void {
    const rect = viewport.getBoundingClientRect();
...
function bindPanel(): void {
  ...
  const nameInput  = document.getElementById('fieldName') as HTMLInputElement;
  const panelName  = document.getElementById('panelAgentName')!;
  const nodeLabel  = document.querySelector('.agent-node-label') as HTMLElement;
  nameInput?.addEventListener('input', () => {
    panelName.textContent = nameInput.value || 'agente';
    if (nodeLabel) nodeLabel.textContent = nameInput.value || 'agente';
  });
  ...
  // Pokémon picker
  const pokemonInput   = document.getElementById('fieldPokemonId') as HTMLInputElement;
  const pickerPreview  = document.getElementById('pokemonPickerPreview') as HTMLImageElement;
  const mainSprite     = document.getElementById('pokemonSprite') as HTMLImageElement;
  const miniSprite     = document.getElementById('panelPokemonMini') as HTMLImageElement;

  function applyPokemonId(id: number): void {
  ...
  pokemonInput?.addEventListener('input', () => {
    ...
    if (!isNaN(val) && val >= 1 && val <= 898) applyPokemonId(val);
  });
}
...
function openPanel(): void {
  ...
  if (pokemonInput) pokemonInput.value = String(currentPokemonId);
